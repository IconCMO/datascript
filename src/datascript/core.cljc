(ns datascript.core
  ...
  (:require
   #?@(:cljs [[goog.array :as garray]
       ...
       :clj  [[clojure.core.async :as a :refer [<! go chan]]]) ;; don't really use core.async in the :clj version, but need this here so everything compiles (since the macros come from the :clj version)
    clojure.walk
  ...
  (-search [db pattern]
           ...
           (let [[e a v tx] pattern
             ...
             (case-tree [e a (some? v) tx]
                        [(<! (eavt (Datom. e a v tx nil)))              ;; e a v tx
                         (<! (eavt (Datom. e a v nil nil)))             ;; e a v _
                         (->> (<! (eavt (Datom. e a nil nil nil)))      ;; e a _ tx
                              (filter (fn [^Datom d] (= tx (.-tx d)))))
                         (<! (eavt (Datom. e a nil nil nil)))           ;; e a _ _
                         (->> (<! (eavt (Datom. e nil nil nil nil)))    ;; e _ v tx
                              (filter (fn [^Datom d] (and (= v (.-v d))
                                                          (= tx (.-tx d))))))
                         (->> (<! (eavt (Datom. e nil nil nil nil)))    ;; e _ v _
                              (filter (fn [^Datom d] (= v (.-v d)))))
                         (->> (<! (eavt (Datom. e nil nil nil nil)))    ;; e _ _ tx
                              (filter (fn [^Datom d] (= tx (.-tx d)))))
                         ...
                         (<! (eavt (Datom. nil nil nil nil nil)))]))))                                             ;; _ _ _ _
